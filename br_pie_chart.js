d3.csv("br_genres.csv").then((data) => {
  //   for (let d of data) {
  //     if (d.weight < 5) {
  //       d.weight = 0;
  //       d.source = " ";
  ...
  document.getElementById("br_pie_chart").appendChild(chart);
});
...
function DonutChart(
  ...
  const I = d3.range(N.length).filter((i) => !isNaN(V[i]) && V[i] > 5);
